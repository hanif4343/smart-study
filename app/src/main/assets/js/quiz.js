...
function toggleAllInSubject(sub, masterCheckbox) {
    ...
    // _getFilteredData — already audience-filtered + cached
    var _sheet = currentMode==='study'?'Study':(currentMode==='quiz'?'Quiz':'QBank');
    var _subject = path[0] || '';
    const data = _getFilteredData(_sheet);
    quizItems = data.filter(function(i) {
        return getVal(i,'subject') === _subject && getVal(i,'sub_topic') === st;
    });
